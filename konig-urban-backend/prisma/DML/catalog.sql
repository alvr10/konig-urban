...
SELECT c.id, cat.id, p.nombre, p.descripcion, p.precio, p.gramaje, p.material, p.img
FROM (
    VALUES
...
SELECT id, fecha_lanzamiento, fecha_lanzamiento + INTERVAL '7 days', TRUE,
...
SELECT 
    p.id,
    CONCAT('UID-', ROW_NUMBER() OVER (), '-', p.nombre),
